...
import { mockValidator, multiKeySignerAddress, smartSessionAddress, timeFramePolicyAddress } from "./utils/constants";
...
export const getContext = async (walletClient: WalletClient, {
  ...
  const uint160Address = BigInt(smartSessionAddress[chainId]);
  ...
  const signerId = keccak256(encodeAbiParameters(parseAbiParameters("address, bytes"), [multiKeySignerAddress[chainId], encodeSigners(signers)]))
  ...
  const actions = permissions.map(permission => (
    {
      ...
      actionPolicies: [{ policy: timeFramePolicyAddress[chainId], initData: encodePacked(["uint128", "uint128"], [BigInt(expiry), BigInt(0)]) }] // hardcoded for demo
    }
  ))
...
export const _prepareMockEnableData = async (
  ...
  let enableData: EnableSessions = {
    isigner: multiKeySignerAddress[chainId], // Example ISigner address
  ...
  enableData.permissionEnableSig = toBytes(encodePacked(['address', 'bytes'], [mockValidator[chainId], await walletClient.signMessage({ account: walletClient.account!, message: digest })]));
  return enableData;
}
...
export const getDigest = async (walletClient: WalletClient, smartAccountAddress: Address, enableData: EnableSessions, mode: SmartSessionMode) => {
  ...
  const contract = new ethers.Contract(smartSessionAddress[chainId], abi, provider);

  const digest = await contract.getDigest(
    multiKeySignerAddress[chainId],
    smartAccountAddress,
    enableData,
    mode
  );
